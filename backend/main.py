...
async def get_favorites(user_id: int):
    ...
    try:
        ...
        result = []
        for f in favorites:
            # image_path DB'de tam URL olarak saklanıyor
            image_url = f[7] if f[7] else ""
            result.append({
                "book_id": f[0],
                "title": f[1],
                "author": f[2],
                ...
                "image_path": image_url,
                "publisher": f[8],
                "user_id": f[9] if f[9] else 0,
                "email": f[10] if f[10] else "",
                "university": f[11] if f[11] else "",
                "department": f[12] if f[12] else "",
