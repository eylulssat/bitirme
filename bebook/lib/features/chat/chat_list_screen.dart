  ...
  Future<void> _fetchChatList() async {
    try {
      final response = await http
          .get(Uri.parse("http://192.168.67.118:8000/chats/${widget.myId}"))
  ...
  Future<void> deleteChat(int otherId, int bookId) async {
    try {
      final response = await http.delete(
        Uri.parse("http://192.168.67.118:8000/chats/delete?my_id=${widget.myId}&other_id=$otherId&book_id=$bookId"),
      );
...
Widget build(BuildContext context) {
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.lock_outline, size: 80, color: primaryColor),
              ),
  ...
  Widget _buildChatCard(dynamic chat, Color primaryColor, BuildContext context) {
    return Container(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(
          ...
          backgroundImage: chat['profile_image'] != null &&
              ...
              ? NetworkImage(
                  "http://192.168.67.118:8000/${chat['profile_image'].toString().replaceAll('\\', '/')}")
              : null,
